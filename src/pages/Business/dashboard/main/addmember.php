<?php require_once(BASEPATH . "/src/pages/Business/dashboard/main/addpay.php"); ?>
<div id="business_addchild_info_modal"
    ...
    <div class="relative bg-white rounded-[30px] shadow-[0px_4px_4px_#0206091A] w-full max-w-[1174px]">
        <!-- Close Button -->
        <button onclick="business_add_info_close_modal()" id="closeModal"
        ...
        <div class="flex flex-col xl:flex-row xl:items-stretch">
            ...
            <div class="px-[16px] sm:px-[30px] py-[56px] w-full xl:w-[60%] ">
                <div class=" max-h-[80vh] overflow-y-auto">
                    <h1 class="font-bold text-[24px] sm:text-[32px] text-[#010205]">Invite Family Member</h1>
                    ...
                    <div class="flex justify-end items-center mt-[26px] gap-[27px]">
                        <span onclick="business_add_info_close_modal()" class="cursor-pointer text-[14px] font-semibold underline text-[#010205]">Cancel</span>
                        <button onclick="inviteMember()"
                            class="w-[172px] h-[44px] rounded-full bg-[#24A556] text-[16px] font-bold text-white">Invite Member</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function business_add_info_close_modal(){
        document.getElementById("business_addchild_info_modal").classList.add("hidden");
    }
            ...
            $.post("/check_status", {
                ...
                if (res.requirePayment) {
                    $.get("/get_discount_price", {}, function(res) {
                        const final_value = parseFloat(res.value) / 100 || 9775;
                        const discount_value = 129 - final_value;
                        $("#family_payment_price_cond").html(`Claim Family discount (${res.price})`);
                        $("#family_payment_price_discount").html("$" + discount_value.toFixed(2));
                        $("#family_payment_price_value").html("$" + final_value.toFixed(2));
                        window.invite_plan_id = res.id;
                        add_payment_show_modal();
                    })
                } else {
                    invite_member();
                }
