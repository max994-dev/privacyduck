...
<div class="flex bg-white justify-center">
            ...
            <div>
                    ...
                    ?>
                        <div>
                            ...
                            ?>
                                <textarea
                                    id="<?php echo $data["id"]; ?>"
                                    class="mt-[6px] rounded-[8px] w-[340px] sm:w-[360px] h-[77px] px-[14px] py-[10px] bg-[#FBFBFB] placeholder:text-[16px] placeholder:leading-[24px] placeholder:text-[#9D9D9D] text-[16px] leading-[24px] text-[#010205]"
                ...
                <div class="flex space-x-2 items-center justify-center">
                    <input type="checkbox"
                        ...
                        id="business_agree_term">&nbsp;&nbsp;
                    <div class="flex flex-wrap">
                        <span class="text-[#010205] text-[16px] leading-[120%]">I would like a live consultation/walkthrough.</span>
                    </div>
                </div>
                <div class="mt-[24px]">
                    ...
                    <button id="business_send"
                        class="hidden flex items-center justify-center w-[340px] sm:w-[360px] h-[44px] rounded-[8px]  bg-[#5AB87F] but-shadow text-center text-[#FAFAFA] font-semibold leading-[24px] text-[16px] 
    ...
    <script>
        ...
        function validation() {
            ids.forEach(id => {
                $("#" + id).on("input", function() {
                    ...
                    if(id == "business_agree_term"){
                        agreed = $(this).is(":checked");
                    }
                    if (!result) {
                        $("#business_email").addClass("valid-border");
                        $("#invalid-business_email").removeClass("hidden").addClass("flex");
                    } else {
                        $("#invalid-business_email").removeClass("flex").addClass("hidden");
                        $("#business_email").removeClass("valid-border");
                    }
                    
                    if (result && ids.every(id => $("#" + id).val().trim() != "")&&agreed) {
        ...
        $("#business_send").click(function() {
            $("#business_send").html(window.loadingHtml);
            $("#business_send").attr("disabled", true);
            const data = {
                "business_first_name": $("#business_first_name").val().trim(),
                "business_last_name": $("#business_last_name").val().trim(),
                "business_email": $("#business_email").val().trim(),
                "business_phone": $("#business_phone").val().trim(),
                "business_message": $("#business_message").val().trim()
            };
            $.post("/business/speaksalesProcess", data, function(res) {
                if(res.error){
                    toastr.error(res.error);
                }else{
                    window.open("<?= WEB_DOMAIN ?>/speaksales/verify", "_self");
                }
                $("#business_send").html("Send Email");
                $("#business_send").attr("disabled", false);
            });
        });
        validation();
    </script>
